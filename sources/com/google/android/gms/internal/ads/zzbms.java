package com.google.android.gms.internal.ads;

import java.util.Collections;
import java.util.Set;
import java.util.concurrent.Executor;
import org.json.JSONObject;

public final class zzbms implements zzdti<Set<zzbuz<zzbrw>>> {

    /* renamed from: a */
    private final zzdtu<zzbmn> f19201a;

    /* renamed from: b */
    private final zzdtu<Executor> f19202b;

    /* renamed from: c */
    private final zzdtu<JSONObject> f19203c;

    private zzbms(zzdtu<zzbmn> zzdtu, zzdtu<Executor> zzdtu2, zzdtu<JSONObject> zzdtu3) {
        this.f19201a = zzdtu;
        this.f19202b = zzdtu2;
        this.f19203c = zzdtu3;
    }

    /* renamed from: a */
    public static zzbms m21374a(zzdtu<zzbmn> zzdtu, zzdtu<Executor> zzdtu2, zzdtu<JSONObject> zzdtu3) {
        return new zzbms(zzdtu, zzdtu2, zzdtu3);
    }

    public final /* synthetic */ Object get() {
        Set set;
        zzbmn zzbmn = this.f19201a.get();
        Executor executor = this.f19202b.get();
        if (this.f19203c.get() == null) {
            set = Collections.emptySet();
        } else {
            set = Collections.singleton(new zzbuz(zzbmn, executor));
        }
        zzdto.m24236a(set, "Cannot return null from a non-@Nullable @Provides method");
        return set;
    }
}
